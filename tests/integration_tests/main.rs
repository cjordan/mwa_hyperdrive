...
use mwa_hyperdrive::{CalibrateUserArgs, CalibrationSolutions};
...
fn serialise_cal_args_toml(args: &CalibrateUserArgs, file: &mut File) {
    let ser = toml::to_string_pretty(&args).expect("couldn't serialise CalibrateUserArgs as toml");
    write!(file, "{}", ser).unwrap();
}

fn serialise_cal_args_json(args: &CalibrateUserArgs, file: &mut File) {
    let ser =
